...
export const SLASetUp = () => {
  ...
  const uploadToApi = async () => {
    setLoading(true);
    try {
      // Upload SLA definitions (first object only)
      const slaPromise =
        slaDefinitions.length > 0
          ? fetch(`${API_URL}api/v1/slas`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(slaDefinitions[0]),
            })
          : Promise.resolve({
              ok: true,
      ...
      // Upload Timer definitions (first object only)
      const timerPromise =
        timerDefinitions.length > 0
          ? fetch(`${API_URL}api/v1/timers`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(timerDefinitions[0]),
            })
          : Promise.resolve({
              ok: true,
              json: async () => ({ success: true, count: 0 }),
      ...
      const [slaResponse, timerResponse] = await Promise.all([
        slaPromise,
        timerPromise,
      ]);

      if (slaResponse.ok && timerResponse.ok) {
        const slaData = await slaResponse.json();
        const timerData = await timerResponse.json();

        setSnackbar({
          open: true,
          message: `Successfully uploaded ${slaDefinitions.length > 0 ? '1 SLA' : '0 SLAs'} and ${timerDefinitions.length > 0 ? '1 Timer' : '0 Timers'} to API!`,
          severity: 'success',
        });
      } else {
