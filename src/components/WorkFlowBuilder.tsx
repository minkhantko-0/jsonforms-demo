...
export function WorkFlowBuilder() {
    ...
    const loadDefinitions = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setAvailableSLAs(
            data.slaDefinitions?.map((s: { key: string }) => s.key) || [],
          );
          setAvailableTimers(
            data.timerDefinitions?.map((t: { key: string }) => t.key) || [],
          );
        }
      } catch (error) {
  ...
  const loadSavedWorkflows = useCallback(async () => {
    try {
      const response = await fetch(`${API_URL}/workflow-definitions`);
      if (response.ok) {
  ...
  const saveWorkflowToApi = useCallback(async () => {
    ...
    try {
      const url = currentWorkflowId
        ? `${API_URL}/workflow-definitions/${currentWorkflowId}`
        : `${API_URL}/workflow-definitions`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ name: workflowName, ...workflowDefinition }),
      });
                  ...
                  <Box
                    ...
                    {savedWorkflows.map(wf => (
                      <Button
                        ...
                        size="small"
                        variant={
                          currentWorkflowId === wf.id ? 'contained' : 'outlined'
                        }
